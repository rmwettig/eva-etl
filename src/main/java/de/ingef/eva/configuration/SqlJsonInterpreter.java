...
public class SqlJsonInterpreter implements JsonInterpreter {
	 ...
	 * @param node Json object representing all queried databases
	...
	public Collection<Query> interpret(JsonNode node) {
		...
		{
			if(_logger != null)_logger.error("'databases' node is empty.");
			return _jobs;
		}
		...
		if(startYear == -1)
		{
			if(_logger != null)_logger.error("Did not found 'startYear' key.");
			return _jobs;
		}
		...
		{
			if(_logger != null) _logger.error("Did not found 'sources' key or is not an array.");
			return _jobs;
		}
		...
		for(JsonNode source : sourcesNode)
		{
			...
			if(!findViews(source, _schema.findDatabaseByName(dbName), years)) continue;
		}
		return _jobs;
	}
	...
	private int extractEndYear(JsonNode dbs) {
		...
		if(node.isMissingNode())
			if(_logger != null)_logger.warn("Did not found '{}' key.\nUsing default: '{}'", "endYear", defaultEndYear);
	 ...
	 * @param source database source entry
	...
	private String findDatabaseName(JsonNode source) {
		...
		{
			if(_logger != null) _logger.error("Did not found source name.");
			return "";
		}
		else
		{
			...
			{
				if(_logger != null) _logger.error("Source name is empty.");
				return "";
			}
			else
	...
	private boolean findViews(JsonNode source, Database schemaDatabase, int[] years) {
		...
		{
			if(_logger != null) _logger.error("Did not found 'views' entry or is not an array");
			return false;
		}
	 ...
	 */
	private boolean processViewByYear(JsonNode source, JsonNode view, String tableName, String yearColumn, Database schemaDatabase,
			int[] years) {
		for(int year : years)
		{
	...
	private boolean processView(JsonNode source, JsonNode view, String tableName, Database schemaDatabase, String partLabel) {
	...
	/**
	 * Adds specified columns to the result set or takes all available from the schema
	 * @param selectParameters parent node that holds the 'columns' entry
	 * @param tableName name of the table for which columns are added
	 * @param schemaColumns available columns
	 * @return false if neither a 'columns' entry is given nor columns exist in the schema
	 */
	...
	private void evaluateWhereEntry(JsonNode selectParameters, String tableName) {
		...
		while(columns.hasNext())
		{
			...
			for(JsonNode condition : columnConditions)
			{
				...
				{
					if(_logger != null) _logger.error("Did not found 'value' entry for condition on table '{}.{}'", tableName, column);
					continue;
				}
				...
				{
					if(_logger != null) _logger.error("Did not found 'operator' entry for condition on table '{}.{}'", tableName, column);
					continue;
				}
				...
				{
					if(_logger != null) _logger.error("Did not found 'type' entry for condition on table '{}.{}'", tableName, column);
					continue;
				}
	...
	private void evaluateJoinEntry(JsonNode selectParameters, String leftTable, Database schemaDatabase) {
		...
		for(JsonNode joinEntry : joinNode)
		{
			...
			{
				if(_logger != null) _logger.error("Did not found 'table' entry for join entry on table '{}'", leftTable);
				continue;
			}
			...
			{
				if(_logger != null) _logger.error("Did not found 'type' entry for join entry on table '{}'", leftTable);
				continue;
			}
			...
			{
				if(_logger != null) _logger.error("Did not found 'on' entry for join entry on table '{}'", table);
				continue;
			}
